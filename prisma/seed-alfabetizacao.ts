// Seed da Trilha de Alfabetização (ConteudoAlfabetizacao/AtividadeFonica) —
// piloto (alfabetizacao-pilot.ts, 3 níveis) + rollout (alfabetizacao-rollout.ts,
// os outros 7). Só toca as tabelas novas da trilha de alfabetização — nunca
// mexe em nada da trilha de Matemática nem do diagnóstico.
//
...
import { ALFABETIZACAO_ROLLOUT } from "./seed-data/alfabetizacao-rollout";
import { obterNivelFonico } from "../src/lib/alfabetizacao";

const TODOS_OS_NIVEIS = [...ALFABETIZACAO_PILOTO, ...ALFABETIZACAO_ROLLOUT];
...
async function main() {
  console.log(`Semeando trilha de alfabetização: ${TODOS_OS_NIVEIS.length} nível(is).`);

  for (const item of TODOS_OS_NIVEIS) {
    ...
    if (existente) {
      ...
      // Diferente do seed-trilha.ts: aqui isso pula o nível em vez de abortar
      // o script inteiro — um rollout que adiciona níveis novos não deve
      // falhar por causa de um nível antigo já em uso de verdade.
      ...
      if (tentativasExistentes > 0 || progressosExistentes > 0) {
        console.log(
          `  ${item.nivel}: pulado — já tem ${tentativasExistentes} tentativa(s) e ${progressosExistentes} progresso(s) de aluno reais registrados.`
        );
        continue;
      }
  ...
  console.log("Seed da trilha de alfabetização concluído com sucesso.");
}
